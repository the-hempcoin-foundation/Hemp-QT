    ...
    if (Mineropret.size() >= PRICES_SIZEBIT0)
    {
        ...
        while (numzero > 0)
        {
            ...
            for (i = numzero = 0; i < n; i++)
            {
                if (pricebits[i] == 0)
                {
                    LogPrintf("zero pricebits i.%d ", i);
                    numzero++;
                }
            }
            if (numzero != 0)
            {
                LogPrintf("komodo_mineropret numzero.%d vs n.%d, zeros in pricebuf, waiting 61 sec for initial prices values loading...\n", numzero, n);
...
void komodo_cbopretupdate(int32_t forceflag)
{
    ...
    {
        while ( pending != 0 )
            LogPrintf("pricewait "), sleep(1);   // reentrant call is waiting until mineropret is updated
        return;  
    }
